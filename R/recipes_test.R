...
ekio_pie <- function(
  ...
  p <- ggplot2::ggplot(
    plot_data,
    ggplot2::aes(x = "", y = percentage, fill = category)
  ) +
...
ekio_boxplot <- function(
  ...
  if (add_ridges && !requireNamespace("ggridges", quietly = TRUE)) {
    warning(
      "ggridges package required for ridges. Install with install.packages('ggridges')"
    )
    add_ridges <- FALSE
  }
...
ekio_slopechart <- function(
  ...
  p <- ggplot2::ggplot(
    data,
    ggplot2::aes(x = !!time_var, y = !!value_var, group = !!group_var)
  ) +
    ggplot2::geom_line(
...
#' @param palette EKIO palette name for colors
#' @param positive_color_index Index for positive values color (default: 5)
#' @param negative_color_index Index for negative values color (default: 4)
#' @param start_color_index Index for start value color (default: 6)
#' @param end_color_index Index for end value color (default: 1)
#' @param show_labels Logical, show value labels (default: TRUE)
#' @param label_accuracy Accuracy for value labels (default: 1)
#' @param theme_style EKIO theme style (default: "modern_premium")
#' @return ggplot2 object
ekio_waterfall <- function(
  data,
  category,
  value,
  start_value = 0,
  palette = "accent_blue",
  positive_color_index = 5,
  negative_color_index = 4,
  start_color_index = 6,
  end_color_index = 1,
  show_labels = TRUE,
  label_accuracy = 1,
  ...
  plot_data <- data |>
    dplyr::mutate(
      category = !!cat_var,
      value = !!val_var,
      col_order = as.numeric(category),
      cumulative = cumsum(dplyr::coalesce(value, 0)),
      yend = cumulative,
      ystart = dplyr::lag(cumulative, default = start_value),
      type = dplyr::case_when(
        col_order == min(col_order) ~ "start",
        col_order == max(col_order) ~ "end",
        value > 0 ~ "positive",
        value < 0 ~ "negative",
        TRUE ~ "total"
      ),
      ystart = if_else(ystart == dplyr::last(ystart), 0, ystart)
    ) |>
    dplyr::mutate(
      color_index = dplyr::case_when(
        type == "positive" ~ positive_color_index,
        type == "negative" ~ negative_color_index,
        type == "start" ~ start_color_index,
        type == "end" ~ end_color_index,
        TRUE ~ NA_integer_
      )
    )

  # If last value is NA then set it to the sum of all previous values
  xvalue <- plot_data$value

  if (is.na(xvalue[length(xvalue)])) {
    xvalue[length(xvalue)] <- sum(xvalue, na.rm = TRUE)
    plot_data$value <- xvalue
  }
