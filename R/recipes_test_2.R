...
ekio_hexmap <- function(
  ...
  if (!requireNamespace("hexbin", quietly = TRUE)) {
    stop(
      "hexbin package required for hexagonal binning. Install with install.packages('hexbin')"
    )
  }
  ...
  if (rlang::quo_is_null(z_var)) {
    ...
    p <- ggplot2::ggplot(
      data,
      ggplot2::aes(x = !!x_var, y = !!y_var, z = !!z_var)
    ) +
...
ekio_parallel <- function(
  ...
  if (use_bumps && !requireNamespace("ggbump", quietly = TRUE)) {
    warning(
      "ggbump package required for smooth bumps. Install with install.packages('ggbump'). Using straight lines instead."
    )
    use_bumps <- FALSE
  }
  ...
  if (!requireNamespace("tidyr", quietly = TRUE)) {
    stop(
      "tidyr package required for data reshaping. Install with install.packages('tidyr')"
    )
  }
  ...
  if (rlang::quo_is_null(group_var)) {
    ...
    if (use_bumps) {
      p <- ggplot2::ggplot(
        plot_data,
        ggplot2::aes(x = variable_num, y = value, group = id)
      ) +
        ggbump::geom_bump(
          alpha = line_alpha,
          size = line_size,
    ...
    if (use_bumps) {
      p <- ggplot2::ggplot(
        plot_data,
        ggplot2::aes(x = variable_num, y = value, group = id, color = group)
      ) +
        ggbump::geom_bump(
          alpha = line_alpha,
          size = line_size
...
ekio_lolli <- function(
  ...
  if (rlang::quo_is_null(color_var)) {
    ...
    if (horizontal) {
      p <- ggplot2::ggplot(
        data,
        ggplot2::aes(x = !!y_var, y = !!x_var, color = !!color_var)
      ) +
        ggplot2::geom_segment(
          ggplot2::aes(x = 0, xend = !!y_var, yend = !!x_var),
          linewidth = line_size
  ...
  if (add_zero) {
    if (horizontal) {
      p <- p +
        ggplot2::geom_vline(xintercept = 0, linewidth = 0.8, color = "gray50")
    } else {
      p <- p +
        ggplot2::geom_hline(yintercept = 0, linewidth = 0.8, color = "gray50")
    }
  }
