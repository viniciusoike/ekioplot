# Package index

## Themes

Apply EKIO visual identity to plots

- [`theme_ekio()`](https://viniciusoike.github.io/ekioplot/reference/theme_ekio.md)
  : Apply EKIO Theme to ggplot2 Plots

## Scales

Color and fill scales for ggplot2. Each topic also documents its fill
and British-spelling aliases (e.g. scale_fill_ekio_d,
scale_colour_ekio_d).

- [`scale_color_ekio_d()`](https://viniciusoike.github.io/ekioplot/reference/scale_color_ekio_d.md)
  [`scale_colour_ekio_d()`](https://viniciusoike.github.io/ekioplot/reference/scale_color_ekio_d.md)
  [`scale_fill_ekio_d()`](https://viniciusoike.github.io/ekioplot/reference/scale_color_ekio_d.md)
  : Discrete Color Scale
- [`scale_color_ekio_c()`](https://viniciusoike.github.io/ekioplot/reference/scale_color_ekio_c.md)
  [`scale_colour_ekio_c()`](https://viniciusoike.github.io/ekioplot/reference/scale_color_ekio_c.md)
  [`scale_fill_ekio_c()`](https://viniciusoike.github.io/ekioplot/reference/scale_color_ekio_c.md)
  : Continuous Color Scale

## Palettes

Access and explore color palettes

- [`ekio_pal()`](https://viniciusoike.github.io/ekioplot/reference/ekio_pal.md)
  : Get Color Palette
- [`list_ekio_palettes()`](https://viniciusoike.github.io/ekioplot/reference/list_ekio_palettes.md)
  : List Available Palettes

## Accessibility

WCAG contrast helpers for readable text on colored fills

- [`ekio_contrast()`](https://viniciusoike.github.io/ekioplot/reference/ekio_contrast.md)
  : WCAG Contrast Ratio Between Two Colors
- [`ekio_text_on()`](https://viniciusoike.github.io/ekioplot/reference/ekio_text_on.md)
  : Pick a Readable Text Color for a Background

## Recipe Functions

High-level chart builders with smart defaults

- [`ekio_histogram()`](https://viniciusoike.github.io/ekioplot/reference/ekio_histogram.md)
  : EKIO Histogram
- [`ekio_lineplot()`](https://viniciusoike.github.io/ekioplot/reference/ekio_lineplot.md)
  : EKIO Line Plot
- [`ekio_scatterplot()`](https://viniciusoike.github.io/ekioplot/reference/ekio_scatterplot.md)
  : EKIO Scatter Plot
- [`ekio_barplot()`](https://viniciusoike.github.io/ekioplot/reference/ekio_barplot.md)
  : EKIO Bar Plot
- [`ekio_areaplot()`](https://viniciusoike.github.io/ekioplot/reference/ekio_areaplot.md)
  : EKIO Area Plot

## Datasets

- [`brazil_population`](https://viniciusoike.github.io/ekioplot/reference/brazil_population.md)
  : Brazilian municipal population, 2025
- [`brazil_population_forecast`](https://viniciusoike.github.io/ekioplot/reference/brazil_population_forecast.md)
  : Brazilian population projections by age and sex, 2025–2100
- [`brazil_gdp`](https://viniciusoike.github.io/ekioplot/reference/brazil_gdp.md)
  : Brazilian municipal GDP, 2021
- [`brazil_agriculture`](https://viniciusoike.github.io/ekioplot/reference/brazil_agriculture.md)
  : Brazilian municipal crop production, 2022
- [`brazil_agriculture_states`](https://viniciusoike.github.io/ekioplot/reference/brazil_agriculture_states.md)
  : Brazilian state crop production, 1974–2023
