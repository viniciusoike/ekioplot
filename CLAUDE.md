...
- **Colors**: defined in `inst/ekio-palettes.yaml` (source of truth), compiled to `R/sysdata.rda` by `data-raw/palettes.R`. Eight nine-step scales — `blue`, `gray`, `stone`, `teal`, `green`, `gold`, `orange`, `red` — named `100`–`900`, light to dark; position `i` == shade `i * 100`. Primary `blue.700` = `#1E3A5F`, ink `gray.900` = `#191A1C`; accents `orange.400` = `#D3742A`, `gold.300` = `#C9AE6B`, `teal.600` = `#006261`. Palettes are defined by `scale.shade` token reference, not literal hex.
...
- **What the spine buys**: shade number means one visual weight in every family, so `blue.500` and `orange.500` are interchangeable. Shade `500` clears WCAG AA on the off-white surface in all eight scales. Both properties are asserted at build time and in `tests/testthat/test-palette-data.R`.
...
- **Surfaces**: the `basic` token group, not a scale — `offwhite` = `#FEFEFE` (the `theme_ekio()` default background), `white` = `#FFFFFF`, `pivot` = `#F5F3EF` (the diverging midpoint), `black` = `#000000`. The `gray.100` tint (`#F2F3F5`) is reachable as `theme_ekio(background = "gray")`. `basic` holds the only hard-coded EKIO hex in the package; it is internal to `.ekio()` and deliberately absent from `ekio_pal()` and `list_ekio_palettes()`
...
- **Unified palette access**: All palette types (categorical, highlight, small-group, scientific, sequential, diverging) go through `ekio_pal()`. Sequential/diverging palettes are also usable in continuous scales
